...
The most up-to date version of this document is available at:
    ...
    https://github.com/aquariuscoin/electrum-aquariuscoin-server/blob/master/HOWTO.md
...
document, we assume that user is called 'aquariuscoin'). We also assume the
aquariuscoin user has sudo rights, so we use '$ sudo command' when we need to.
...
you also need to run aquariuscoind and keep a copy of the full blockchain, 
which is roughly 4 GB in July 2015. If you have less than 2 GB of RAM 
make sure you limit aquariuscoind to 8 concurrent connections. If you have more 
...
### Step 1. Create a user for running aquariuscoind and Electrum server

This step is optional, but for better security and resource separation I
suggest you create a separate user just for running `aquariuscoind` and Electrum.
We will also use the `~/bin` directory to keep locally installed files
(others might want to use `/usr/local/bin` instead). We will download source
code files to the `~/src` directory.

    $ sudo adduser aquariuscoin --disabled-password
    $ sudo apt-get install git
    $ sudo su - aquariuscoin
    $ mkdir ~/bin ~/src
    $ echo $PATH

If you don't see `/home/aquariuscoin/bin` in the output, you should add this line
to your `.bashrc`, `.profile`, or `.bash_profile`, then logout and relogin:

    PATH="$HOME/bin:$PATH"
    $ exit

### Step 2. Download aquariuscoind

We currently recommend aquariuscoind 0.10.2.2 stable.

If you prefer to compile aquariuscoind, here are some pointers for Ubuntu:
    ...
    $ sudo su - aquariuscoin
    $ cd ~/src && git clone https://github.com/aquariuscoin-project/aquariuscoin.git
    $ cd aquariuscoin/src
    $ make -f makefile.unix
    $ strip aquariuscoind
    $ cp -a aquariuscoind ~/bin

### Step 3. Configure and start aquariuscoind

In order to allow Electrum to "talk" to `aquariuscoind`, we need to set up an RPC
username and password for `aquariuscoind`. We will then start `aquariuscoind` and
wait for it to complete downloading the blockchain.

    $ mkdir ~/.aquariuscoin
    $ $EDITOR ~/.aquariuscoin/aquariuscoin.conf

Write this in `aquariuscoin.conf`:
...
If you have an existing installation of aquariuscoind and have not previously
set txindex=1 you need to reindex the blockchain by running

    $ aquariuscoind -reindex

If you already have a freshly indexed copy of the blockchain with txindex start `aquariuscoind`:

    $ aquariuscoind

Allow some time to pass, so `aquariuscoind` connects to the network and starts
downloading blocks. You can check its progress by running:

    $ aquariuscoind getinfo

Before starting the electrum server your aquariuscoind should have processed all
blocks and caught up to the current height of the network (not just the headers).
You should also set up your system to automatically start aquariuscoind at boot
time, running as the 'aquariuscoin' user. Check your system documentation to
...
We will download the latest git snapshot for Electrum to configure and install it:
    ...
    $ cd ~
    $ git clone https://github.com/aquariuscoin/electrum-aquariuscoin-server.git
    $ cd electrum-aquariuscoin-server
    $ sudo ./configure
...
http://electrum1.aquariuscoin.org/leveldb-dump/
...
Electrum reads a config file (/etc/electrum-aquariuscoin.conf) when starting up. This
file includes the database setup, aquariuscoind RPC setup, and a few other
options.

The "configure" script listed above will create a config file at /etc/electrum-aquariuscoin.conf
...
The "configure" script will take care of this and ask you to create a user for running electrum-aquariuscoin-server.
If you're using user aquariuscoin to run electrum and have added it manually like shown in this HOWTO run 
the following code to add the limits to your /etc/security/limits.conf:

     echo "aquariuscoin hard nofile 65536" >> /etc/security/limits.conf
     echo "aquariuscoin soft nofile 65536" >> /etc/security/limits.conf
...
Or if you use sudo and the user is added to sudoers group:
    ...
    sudo -u aquariuscoin -i ulimit -n


Two more things for you to consider:

1. To increase security you may want to close aquariuscoind for incoming connections and connect outbound only

2. Consider restarting aquariuscoind (together with electrum-aquariuscoin-server) on a weekly basis to clear out unconfirmed
...
Note: If you want to run the server without installing it on your system, just run 'run_electrum_aquariuscoin_server" as the
unprivileged user.
...
If your system supports it, you may add electrum-aquariuscoin-server to the /etc/init.d directory. 
...
aquariuscoins to confirm that everything is working properly.

### Step 13. Join us on IRC, subscribe to the server thread

Say hi to the dev crew, other server operators, and fans on
irc.freenode.net #electrum-aquariuscoin and we'll try to congratulate you
...
https://groups.google.com/forum/#!forum/electrum-aquariuscoin-server
